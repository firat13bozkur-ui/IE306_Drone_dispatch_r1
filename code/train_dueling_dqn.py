...
from dqn_agent import build_battery_aware_action_mask, masked_argmax, sample_valid_action
...
class DuelingDQNAgent:
    """
    Dueling DQN agent with action masking.
    """
...
def dueling_double_dqn_train_step(
    agent,
    target_network,
    replay_buffer,
    optimizer,
    ...
    """
    Dueling network architecture + Double DQN target.

    Online network selects next action.
    Target network evaluates selected next action.
    """
    ...
    with torch.no_grad():
        next_q_online = agent.q_network(next_states)
        next_q_online = next_q_online.masked_fill(~next_action_masks, -1e9)
        next_actions = next_q_online.argmax(dim=1)

        next_q_target = target_network(next_states)
        selected_next_q_values = next_q_target.gather(
            1,
            next_actions.unsqueeze(1),
        ).squeeze(1)

        target_q_values = rewards + gamma * (1.0 - dones) * selected_next_q_values
...
def main():
    ...
    agent = DuelingDQNAgent(
        ...
        device=device,
        prefer_assignment=bool(cfg_train.get("prefer_assignment", True)),
        charge_threshold=float(cfg_train.get("charge_threshold", 0.0)),
    )
        ...
        for episode in range(total_episodes):
            ...
            while not done:
                ...
                if (
                    ...
                    loss = dueling_double_dqn_train_step(
                        agent=agent,
